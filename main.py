...
async def create_jobs(jobs: jobPosting):
    jobs_dict = jobs.dict()
    result = await collection.insert_one(jobs_dict)
...
async def create_applicants(candidates: applicants):
    ...
    result = await collection.insert_one(candidates_dict)
...
# Read all applicants
@app.get("/applicants", response_model=list[companyDB])
async def get_applicant():
    applicants = []
    async for applicant in collection.find():
...
async def update_company(company_id: str, Company: company):
    ...
    if result.modified_count == 0:
        raise HTTPException(404, "User not found")
    updated = await collection.find_one({"_id": ObjectId(company_id)})
